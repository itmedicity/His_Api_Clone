...
module.exports = {
  ...
  medicineImportedDateUpdate: (data, callBack) => {
    pool.query(`update medi_ellider.meddesc_lastupdate set lastupdate = ? where sl_no=1`, [data.lastupdate], (error, results, feilds) => {
      if (error) {
        return callBack(error);
      }
  ...
  getImportedDate: (callBack) => {
    pool.query(`select lastupdate from  medi_ellider.meddesc_lastupdate where sl_no=1`, (error, results, feilds) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  getMedicinesFromMysql: (callBack) => {
    pool.query(`select * from medicine_descriptions order by itc_desc`, (error, results, feilds) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  medicineDetailsUpdate: (data, callBack) => {
    pool.query(`update medi_ellider.medicine_descriptions set  itn_strip = ?, itn_pack = ? where it_code = ?`, [data.itn_strip, data.itn_pack, data.it_code], (error, results, feilds) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
