...
module.exports = {
  ...
  advanceCollectionTmch: async (conn_ora, data) => {
    ...
    const sql = `SELECT SUM (NVL (ARN_AMOUNT, 0)) Amt, 0 tax
                        ...
                        WHERE (NVL (ARC_CANCEL, 'N') = 'N')
                                ...
                                AND NOT EXISTS (
                                  SELECT 1 FROM GTT_EXCLUDE_IP G WHERE G.IP_NO = IPADVANCE.IP_NO
                                )`;
  ...
  advanceRefundTmch: async (conn_ora, data) => {
  ...
  //Advance Settled
  advanceSettledTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
    ...
    const advanceSettledSql = `SELECT SUM (NVL (Opn_advance, 0)) Amt, 0 tax
                                    ...
                                    WHERE NVL (BMC_CANCEL, 'N') = 'N'
                                            ...
                                            AND NOT EXISTS (
                                              SELECT 1 FROM GTT_EXCLUDE_IP G WHERE G.IP_NO = PBILLMAST.IP_NO
                                            )
                                    ...
                                    WHERE NVL (BMC_CANCEL, 'N') = 'N'
                                            ...
                                            AND Bmd_Date <= TO_DATE (:toDate, 'dd/MM/yyyy hh24:mi:ss')
                                            AND NOT EXISTS (
                                            SELECT 1 FROM GTT_EXCLUDE_IP G WHERE G.IP_NO = BILLMAST.IP_NO
                                            )
  ...
  collectionAgainstSalePart1Tmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  collectionAgainstSalePart2Tmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  creditInsuranceBillCollectionTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  creditInsuranceBillTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  ipConsolidatedDiscountTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  ipPreviousDayDiscountTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  ipPreviousDayCollectionTmch: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  getIpNumberFromPreviousDayCollection: (data) => {
    return query(
      "ellider",
      `SELECT ip_no,dis_status,tmch_status
        FROM tssh_ipadmiss 
        WHERE ip_no IN (?)`,
      [data],
    );
