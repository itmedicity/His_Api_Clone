...
const getProcedureDetl_TMCH = async (conn_ora, bind) => {
  const sql = `WITH EXCLUDE_IP AS (
    SELECT /*+ MATERIALIZE */ IP_NO FROM MEDIWARE.GTT_EXCLUDE_IP WHERE STATUS = 1
),
CTE_DISBILL AS (
    SELECT d.*
    FROM DISBILLMAST d
...
CTE_INCEXP AS (
    SELECT /*+ MATERIALIZE */
        ...
        md.DG_GRCODE,
        mg.DG_DESC
    FROM MISINCEXPDTL md
         JOIN MISINCEXPGROUP mg
             ON mg.DG_GRCODE = md.DG_GRCODE
    WHERE md.DG_TYPE = 'R'
),
CTE_RECEIPT AS (
    SELECT r.*
    ...
    WHERE NVL(r.RPC_CANCEL, 'N') <> 'C'
      AND r.RPC_COLLCNCODE IS NULL
      AND r.RPC_CACR IN ('C', 'R')
      AND r.RPD_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
      AND r.RPD_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
),
CTE_RECEIPT_COLL AS (
    SELECT r.*
    FROM RECEIPTMAST r
         JOIN MULTIHOSPITAL mh
           ON mh.MH_CODE = r.MH_CODE
    WHERE r.RPC_CANCEL IS NULL
      AND r.RPC_COLLCNCODE IS NOT NULL
      AND r.RPC_CACR IN ('C','R')
      AND r.RPD_COLLDATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
      AND r.RPD_COLLDATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
),
CTE_OPBILL AS (
    SELECT o.*
    FROM OPBILLMAST o
    ...
    WHERE NVL(o.OPN_CANCEL,'N') = 'N'
      AND o.OPC_CACR <> 'M'
      AND o.OPD_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
      AND o.OPD_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
),
CTE_BILL AS (
    SELECT b.*
    FROM BILLMAST b
    ...
    WHERE b.BMC_CACR IN ('C','R')
      AND NVL(b.BMC_CANCEL,'N') <> 'C'
      AND b.BMC_COLLCNCODE IS NULL
      AND b.BMD_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
      AND b.BMD_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
)
SELECT  
    PCC_DESC,CODE,SUM(AMT) AMT,SUM(TAXAMT) TAXAMT,EX.DG_DESC
FROM (
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(ps.SVN_QTY * ps.SVN_RATE, 0) - NVL(ps.SVN_DISAMT, 0) ) AS AMT,
    SUM(NVL(ps.SVN_TOTTAX, 0)) AS TAXAMT
FROM PATSERVICE ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = ps.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
...
UNION ALL
    ...
    SUM(NVL(pv.VSN_TOTTAX, 0)) AS TAXAMT
FROM PATVISIT pv
JOIN CTE_DISBILL d ON d.DMC_SLNO = pv.DMC_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_VSCODE
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(dr.RDN_AMOUNT * dr.RDN_DAYS, 0)  - NVL(dr.RDN_DISAMT, 0)) AS AMT,
    SUM(NVL(dr.RDN_TOTTAX, 0)) AS TAXAMT
FROM DISRMRENTDETL dr
JOIN CTE_DISBILL d ON d.DMC_SLNO = dr.DMC_SLNO
JOIN CTE_INCEXP ie ON ie.PC_CODE = dr.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
GROUP BY pc.PC_CODE, pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
...
JOIN CTE_DISBILL d ON d.DMC_SLNO = dr.DMC_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_NUCODE
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE,
    SUM(NVL(rid.RIN_NETAMT,0)) * -1 AMT,
    SUM(NVL(rid.RIN_TOTTAX,0)) * -1 TAXAMT
FROM IPREFUNDMAST rm
JOIN IPREFUNDITEMDETL rid ON rid.RIC_SLNO = rm.RIC_SLNO
LEFT JOIN EXCLUDE_IP e ON e.IP_NO = rm.IP_NO
JOIN CTE_INCEXP ie ON ie.PC_CODE = rid.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = rid.PC_CODE
JOIN MULTIHOSPITAL mh ON mh.MH_CODE = rm.MH_CODE
WHERE e.IP_NO IS NULL
  AND rm.RIC_CACR IN ('C','R')
  AND NVL(rm.RIC_CANCEL,'N')='N'
  AND rid.RIC_TYPE <> 'PHY'
  AND rm.RID_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
  AND rm.RID_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM( NVL(ps.SRN_OPERATION, 0) - NVL(ps.SRN_OPERDIS, 0)) AS AMT,
    SUM(NVL(ps.SRN_OPERTOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_OPER
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(ps.SRN_THEATER, 0) - NVL(ps.SRN_THEARDIS, 0)) AS AMT,
    SUM(NVL(ps.SRN_THEATTOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_THER
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(ps.SRN_CHIEF, 0)) AS AMT,
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_CHIEF
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_1STASST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(ps.SRN_2NDASST, 0)) AS AMT,
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_2NDASST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_3RDASST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(ps.SRN_GUEST, 0)) AS AMT,
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_GUEST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM( NVL(ps.SRN_ANTEST, 0) - NVL(ps.SRN_ANTDIS, 0)) AS AMT,
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_ANEST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_ANEST2
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(po.PDN_AMOUNT, 0)) AS AMT,
    SUM(NVL(po.PSN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSUROTHER po ON po.SR_SLNO = ps.SR_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = po.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(po.SRC_CANCEL, 'N') = 'N'
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(psd.SRN_AMOUNT, 0) - NVL(psd.SRN_DISCOUNT, 0)) AS AMT,
    SUM(NVL(psd.PSN_TOTTAX, 0)) AS TAXAMT
FROM PATSURGERY ps
JOIN CTE_DISBILL d ON d.DMC_SLNO = ps.DMC_SLNO
JOIN PATSURDETL psd ON psd.SR_SLNO = ps.SR_SLNO
JOIN CTE_INCEXP ie ON ie.PC_CODE = psd.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = psd.PC_CODE
WHERE NVL(ps.SRC_CANCEL, 'N') = 'N'
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(cb.CMN_NETAMT, 0) - NVL(cb.CMN_OUTCOLL, 0)) AS AMT,
    0 AS TAXAMT
FROM CANBILLMAST cb
JOIN CTE_DISBILL d ON d.DMC_SLNO = cb.DMC_SLNO
JOIN IPPARAM ip ON ip.MH_CODE = d.MH_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = ip.IPC_CANTEENCODE
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(cb.CMC_CANCEL, 'N') = 'N'
  AND cb.CMC_CACR = 'I'
GROUP BY pc.PC_CODE, pc.PCC_DESC
UNION ALL
SELECT
    ...
    SUM(NVL(ps.SRN_OPERTOTTAX,0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.OPERATION_OPSLNO = op.OPC_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_OPER
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_OPERATION,0) > 0
  AND NVL(ps.SRC_CANCEL,'N') = 'N'
  AND NVL(op.OPN_CANCEL,'N') = 'N'
  AND op.OPC_CACR <> 'M'
  AND op.OPD_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
  AND op.OPD_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
  AND op.MH_CODE IN (SELECT MH_CODE FROM MULTIHOSPITAL)
  AND ex.IP_NO IS NULL
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    ...
    SUM(NVL(ps.SRN_THEATTOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.THEATER_OPSLNO = op.OPC_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_THER
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_THEATER, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.CHIEF_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_CHIEF
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_CHIEF, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.D1STASST_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_1STASST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_1STASST, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.D2NDASST_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_2NDASST
JOIN PROCATEGORY pc
    ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_2NDASST, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
  AND op.OPC_CACR <> 'M'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.D3RDASST_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_3RDASST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_3RDASST, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.GUEST_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_GUEST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_GUEST, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.ANTEST_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_ANEST
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_ANTEST, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(pr.SRN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURGERY ps ON ps.ANTEST2_OPSLNO = op.OPC_SLNO
JOIN PATSURGERYRESOURCESDETL pr ON pr.SR_SLNO = ps.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
CROSS JOIN OPPARAM opp
JOIN CTE_INCEXP ie ON ie.PC_CODE = opp.OPC_ANEST2 
JOIN PROCATEGORY pc ON pc.PC_CODE = ie.PC_CODE
WHERE NVL(ps.SRN_ANTEST2, 0) > 0
  AND NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
...
SELECT
    ...
    SUM(NVL(po.PSN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSUROTHER po ON po.OPC_SLNO = op.OPC_SLNO
JOIN PATSURGERY ps ON ps.SR_SLNO = po.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = po.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM( NVL(pd.SRN_AMOUNT, 0) - NVL(pd.SRN_DISCOUNT, 0)) AS AMT,
    SUM(NVL(pd.PSN_TOTTAX, 0)) AS TAXAMT
FROM OPBILLMAST op
JOIN PATSURDETL pd ON pd.OPC_SLNO = op.OPC_SLNO
JOIN PATSURGERY ps ON ps.SR_SLNO = pd.SR_SLNO
LEFT JOIN EXCLUDE_IP ex ON ex.IP_NO = ps.IP_NO
JOIN CTE_INCEXP ie ON ie.PC_CODE = pd.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pd.PC_CODE
WHERE NVL(ps.SRC_CANCEL, 'N') = 'N'
  AND NVL(pd.SRC_CANCEL, 'N') = 'N'
  AND NVL(op.OPN_CANCEL, 'N') = 'N'
  AND op.OPC_CACR <> 'M'
  AND op.OPD_DATE >= TO_DATE(:fromDate, 'dd/MM/yyyy hh24:mi:ss')
  AND op.OPD_DATE <= TO_DATE(:toDate, 'dd/MM/yyyy hh24:mi:ss')
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(rd.RPN_NETAMT, 0)) AS AMT,
    SUM(NVL(rd.RPN_TOTTAX, 0)) AS TAXAMT
FROM CTE_RECEIPT rm
JOIN RECEIPTDETL rd ON rd.RPC_SLNO = rm.RPC_SLNO
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(rd.RPN_NETAMT,0)) AMT,
    SUM(NVL(rd.RPN_TOTTAX,0)) TAXAMT
FROM CTE_RECEIPT_COLL rm
JOIN RECEIPTDETL rd ON rd.RPC_SLNO = rm.RPC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
...
FROM RECEIPTMAST rm
JOIN RECEIPTDETL rd ON rd.RPC_SLNO = rm.RPC_SLNO
JOIN CTE_OPBILL op ON op.OPC_SLNO = rd.OPC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE rm.RPC_CANCEL IS NULL AND rm.RPC_CACR = 'O'
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(rrd.RPN_NETAMT, 0)) * -1 AS AMT,
    SUM(NVL(rrd.RFN_TOTTAX, 0)) * -1 AS TAXAMT
FROM REFUNDRECEIPTMAST rrm
JOIN REFUNDRECEIPTDETL rrd ON rrd.RFC_SLNO = rrm.RFC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rrd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
JOIN MULTIHOSPITAL mh ON mh.MH_CODE = rrm.MH_CODE
WHERE NVL(rrm.RFC_CANCEL, 'N') <> 'C'
  AND rrm.RFC_RETCNCODE IS NULL
  AND rrm.RFC_CACR IN ('C','R')
  AND rrd.RFD_DATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
  AND rrd.RFD_DATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
...
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE rrm.RFC_RETCNCODE IS NOT NULL
  AND rrm.RFC_CANCEL IS NULL
  AND rrm.RFC_CACR IN ('C', 'R')
  AND rrm.RFD_RETDATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
  AND rrm.RFD_RETDATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    ...
    SUM(NVL(rrd.RFN_TOTTAX, 0)) * -1 AS TAXAMT
FROM CTE_OPBILL op
JOIN RECEIPTDETL rd ON rd.OPC_SLNO = op.OPC_SLNO
JOIN REFUNDRECEIPTDETL rrd ON rrd.RPC_SLNO = rd.RPC_SLNO AND rrd.RPC_CNT  = rd.RPC_CNT
JOIN REFUNDRECEIPTMAST rrm ON rrm.RFC_SLNO = rrd.RFC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rrd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE NVL(rrd.RFC_CANCEL, 'N') = 'N'
  AND rrm.RFC_CACR = 'O'
GROUP BY pc.PC_CODE, pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM( (NVL(bd.PDN_RATE, 0) * NVL(bd.PDN_QTY, 0)) - NVL(bd.BMN_DISAMT, 0) ) AS AMT,
    SUM(NVL(bd.BDN_TOTTAX, 0)) AS TAXAMT
FROM CTE_BILL bm
JOIN BILLDETL bd ON bd.BMC_SLNO = bm.BMC_SLNO
...
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM( (NVL(bd.PDN_RATE,0) * NVL(bd.PDN_QTY,0)) - NVL(bd.BMN_DISAMT,0)) AMT,
    SUM(NVL(bd.BDN_TOTTAX,0)) TAXAMT
FROM BILLMAST bm
JOIN MULTIHOSPITAL mh ON mh.MH_CODE = bm.MH_CODE
JOIN BILLDETL bd ON bd.BMC_SLNO = bm.BMC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = bd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
...
WHERE bm.BMC_COLLCNCODE IS NOT NULL
  AND bm.BMC_CACR IN ('C','R')
  AND NVL(bm.BMC_CANCEL,'N') = 'N'
  AND bm.BMD_COLLDATE >= TO_DATE(:fromDate,'dd/MM/yyyy hh24:mi:ss')
  AND bm.BMD_COLLDATE <= TO_DATE(:toDate,'dd/MM/yyyy hh24:mi:ss')
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    ...
    SUM(NVL(bd.BDN_TOTTAX, 0)) AS TAXAMT
FROM CTE_OPBILL op
JOIN BILLDETL bd ON bd.OPC_SLNO = op.OPC_SLNO
JOIN BILLMAST bm ON bm.BMC_SLNO = bd.BMC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = bd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE bm.BMC_CACR = 'O' AND NVL(bm.BMC_CANCEL, 'N') = 'N'
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    ...
    SUM(NVL(bd.BDN_TOTTAX, 0)) AS TAXAMT
FROM CTE_DISBILL db
JOIN BILLMAST bm ON bm.DMC_SLNO = db.DMC_SLNO
JOIN BILLDETL bd ON bd.BMC_SLNO = bm.BMC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = bd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
...
SELECT
    ...
    pc.PC_CODE AS CODE,
    SUM(NVL(rbd.RFN_NETAMT, 0)) * -1 AS AMT,
    SUM(NVL(rbd.RFN_TOTTAX, 0)) * -1 AS TAXAMT
FROM REFUNDBILLDETL rbd
JOIN REFUNDBILLMAST rbm ON rbm.RFC_SLNO = rbd.RFC_SLNO
JOIN BILLMAST bm ON bm.BMC_SLNO = rbm.BMC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rbd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
JOIN MULTIHOSPITAL mh ON mh.MH_CODE = bm.MH_CODE
...
FROM CTE_OPBILL op
JOIN BILLDETL bd ON bd.OPC_SLNO = op.OPC_SLNO
JOIN REFUNDBILLDETL rbd ON rbd.BMC_SLNO = bd.BMC_SLNO AND rbd.BMC_CNT  = bd.BMC_CNT
JOIN REFUNDBILLMAST rbm ON rbm.RFC_SLNO = rbd.RFC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rbd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE rbm.RFC_CACR = 'O' AND NVL(rbd.RFC_CANCEL, 'N') = 'N'
GROUP BY pc.PC_CODE,pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
...
FROM CTE_DISBILL db
JOIN BILLMAST bm ON bm.DMC_SLNO = db.DMC_SLNO
JOIN REFUNDBILLDETL rbd ON rbd.BMC_SLNO = bm.BMC_SLNO
JOIN REFUNDBILLMAST rbm ON rbm.RFC_SLNO = rbd.RFC_SLNO
JOIN PRODESCRIPTION pd ON pd.PD_CODE = rbd.PD_CODE
JOIN PROGROUP pg ON pg.PG_CODE = pd.PG_CODE
JOIN CTE_INCEXP ie ON ie.PC_CODE = pg.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = pg.PC_CODE
WHERE rbm.RFC_CACR = 'I' AND NVL(rbd.RFC_CANCEL, 'N') = 'N'
GROUP BY pc.PC_CODE, pc.PCC_DESC
UNION ALL
SELECT
    pc.PCC_DESC,
    pc.PC_CODE AS CODE,
    SUM(NVL(dd.DSN_PACKAGEADJAMT, 0)) AS AMT,
    0 AS TAXAMT
FROM CTE_DISBILL db
JOIN DISBILLDETL dd ON dd.DMC_SLNO = db.DMC_SLNO
JOIN CTE_INCEXP ie ON ie.PC_CODE = dd.PC_CODE
JOIN PROCATEGORY pc ON pc.PC_CODE = dd.PC_CODE
GROUP BY pc.PC_CODE, pc.PCC_DESC
) A JOIN CTE_INCEXP EX  ON  EX.PC_CODE = A.CODE  GROUP BY PCC_DESC,CODE,EX.DG_DESC`;
  const result = await conn_ora.execute(
    sql,
    {
      fromDate: bind.from,
