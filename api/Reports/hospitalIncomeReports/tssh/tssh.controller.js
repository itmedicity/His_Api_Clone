...
const getTsshReport = async (req, res) => {
  let conn;
  try {
    ...
    const ipPreviousDayCollection = async (conn, bind) => {
      ...
      const getResult = await getIpNumberFromPreviousDayCollection(postData);
...
const getTsshCeditInsuranceBillCollection = async (req, res) => {
  let conn;
  try {
    const body = req.body;
    // console.log(body);
...
const getTsshCreditInsuranceBillDetail = async (req, res) => {
  let conn;
  try {
    const body = req.body;
    // console.log(body);
    ...
    const result = await qmtService.get_CreditInsuranceBill(conn, body);
...
const getTssh_UnsettledAmount = async (req, res) => {
  let conn;
  try {
    const body = req.body;
    // console.log(body);
    ...
    const result = await qmtService.get_UnsettledAmount_TSSH(conn, body);
    // console.log(result);s
    if (!result || (Array.isArray(result) && result.length === 0)) {
      return res.status(200).json({
        success: 2,
...
const getTssh_AdvanceCollection = async (req, res) => {
  let conn;
  try {
    const body = req.body;
    // console.log(body);
    ...
    const result = await qmtService.get_AdvanceCollection_TSSH(conn, body);
    // console.log(result);s
    if (!result || (Array.isArray(result) && result.length === 0)) {
      return res.status(200).json({
        success: 2,
...
const getTssh_ProcedureDetails = async (req, res) => {
  let conn;
  try {
    const body = req.body;
    // console.log(body);
    ...
    const result = await qmtService.get_ProcedureDetails_TSSH(conn, body);
    // console.log(result);s
    if (!result || (Array.isArray(result) && result.length === 0)) {
      return res.status(200).json({
        success: 2,
...
module.exports = {getTsshReport, getTsshCeditInsuranceBillCollection, getTsshCreditInsuranceBillDetail, getTssh_UnsettledAmount, getTssh_AdvanceCollection, getTssh_ProcedureDetails};
