...
module.exports = {
  ...
  ipConsolidatedDiscountTssh: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  ipPreviousDayDiscountTssh: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
  ...
  ipPreviousDayCollectionTssh: async (conn_ora, data) => {
    // let conn_ora = await getTmcConnection();
    // const ipNumberList = (data?.ipNoColl?.length > 0 && data.ipNoColl.join(",")) || null;
    // const fromDate = data.from;
    // const toDate = data.to;
  ...
  creditInsuranceBillRefund: async (conn_ora) => {
    // let conn_ora = await getTmcConnection();
    // const fromDate = data.from;
    // const toDate = data.to;
  ...
  getIpNumberFromPreviousDayCollection: async (data) => {
    const sql = `SELECT ip_no,dis_status,tmch_status
        FROM tssh_ipadmiss 
        WHERE ip_no IN (?) AND tmch_status = ?`;
    return await query("ellider", sql, [data.ipno, data.group]);
  },
};
