// cronLogger.js
const {tr} = require("date-fns/locale");
const {pools, query} = require("../config/mysqldbconfig");
// const mysqlpool = require("../config/dbconfigmeliora");

/* --------------------------------------------------
   SINGLE QUERY
-------------------------------------------------- */

const mysqlExecute = async (sql, values = []) => {
  const [rows] = await pools.meliora.execute(sql, values);
  return rows;
};

/* --------------------------------------------------
   START LOG
-------------------------------------------------- */

const startLog = async (cronName) => {
  const [result] = await pools.meliora.execute(
    `INSERT INTO cron_job_log (cron_name, start_time)
     VALUES (?, NOW())`,
    [cronName],
  );
  return result.insertId;
};

/* --------------------------------------------------
   SUCCESS LOG
-------------------------------------------------- */

const endLogSuccess = async (logId, data = {}) => {
  const {oracleRows = 0, mysqlInserted = 0, mysqlUpdated = 0, oracleTime = null, mysqlTime = null} = data;

  await pools.meliora.execute(
    `UPDATE cron_job_log
       SET end_time = NOW(),
           oracle_rows_fetched = ?,
           mysql_inserted = ?,
           mysql_updated = ?,
           oracle_fetch_time_ms = ?,
           mysql_write_time_ms = ?,
           status = 'SUCCESS'
       WHERE id = ?`,
    [oracleRows, mysqlInserted, mysqlUpdated, oracleTime, mysqlTime, logId],
  );
};

/* --------------------------------------------------
   FAILURE LOG
-------------------------------------------------- */

const endLogFailure = async (logId, error) => {
  await pools.meliora.execute(
    `UPDATE cron_job_log
       SET status = 'FAILED',
           error_message = ?,
           end_time = NOW()
       WHERE id = ?`,
    [error.message || String(error), logId],
  );
};

/* --------------------------------------------------
   COMPANY SLNO // Fetching Current company Slno
-------------------------------------------------- */

const getCompanySlno = async () => {
  const rows = await query("meliora", "SELECT company_slno FROM crm_common LIMIT 1");
  if (!rows.length) {
    throw new Error("company_slno not found in crm_common");
  }
  return rows[0].company_slno;
};

/* --------------------------------------------------
   LAST TRIGGER DATE
-------------------------------------------------- */

const getLastTriggerDate = async (processId) => {
  const [rows] = await pools.meliora.execute(
    `SELECT fb_last_trigger_date
     FROM fb_ipadmiss_logdtl
     WHERE fb_process_id = ?`,
    [processId],
  );

  return rows.length ? rows[0] : null;
};

/* --------------------------------------------------
   TRANSACTION (CRON SAFE) // MySql transaction for patient insert Query
-------------------------------------------------- */
const mysqlExecuteTransaction = async (queries = []) => {
  const conn = await pools.meliora.getConnection();

  try {
    await conn.beginTransaction();

    const results = [];
    for (const q of queries) {
      const [res] = await conn.query(q.sql, q.values || []);
      results.push(res);
    }

    await conn.commit();
    return results;
  } catch (err) {
    await conn.rollback();
    throw err;
  } finally {
    conn.release();
  }
};

/* --------------------------------------------------
   LAST PROCESSED ADMISSION DATE
-------------------------------------------------- */

const getLastProcessedAdmissionDate = async () => {
  //  GET MAX ADMISSION DATE FROM IPADMISS
  const [rows] = await pools.meliora.execute(`SELECT MAX(fb_ipd_date) AS last_admitted_date FROM fb_ipadmiss`);
  return rows[0]?.last_admitted_date ? new Date(rows[0].last_admitted_date) : null;
};

/* --------------------------------------------------
   LAST PROCESSED DISCHARGE DATE
-------------------------------------------------- */
const getLastProcessedDischargeDate = async () => {
  const [rows] = await pools.meliora.execute(`SELECT MAX(fb_dmd_date) AS last_discharge_date FROM fb_ipadmiss`);
  return rows[0]?.last_discharge_date ? new Date(rows[0].last_discharge_date) : null;
};

/* --------------------------------------------------
   DISCHARGE HISTORY
-------------------------------------------------- */

const captureDischargeHistory = async (row) => {
  const [rows] = await pools.meliora.execute(
    `SELECT fb_ipc_curstatus, fb_dmd_date, fb_do_code, fb_doc_name
     FROM fb_ipadmiss
     WHERE fb_ip_no = ?`,
    [row.IP_NO],
  );

  if (!rows.length) return;

  const old = rows[0];

  if (!row.DMD_DATE || new Date(row.DMD_DATE) <= new Date(old.fb_dmd_date)) return;

  await pools.meliora.execute(
    `INSERT INTO fb_ipadmiss_status_history (
      fb_ip_no, old_status, new_status,
      old_dmd_date, new_dmd_date,
      old_do_code, new_do_code,
      old_doc_name, new_doc_name
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [row.IP_NO, old.fb_ipc_curstatus, row.IPC_CURSTATUS, old.fb_dmd_date, row.DMD_DATE, old.fb_do_code, row.DO_CODE, old.fb_doc_name, row.DOC_NAME],
  );
};

/* --------------------------------------------------
   GET SCHEMA BY COMPANY AND MODULE
-------------------------------------------------- */

const getSchemaByCompanyAndModule = async (companySlno, moduleCode) => {
  const schemaDetail = await pools.meliora.execute(
    `
        SELECT schema_name
        FROM hsp_company_schema_map
        WHERE hsp_company_slno = ?
          AND hsp_module_code  = ?
          AND hsp_map_status = 1
        `,
    [companySlno, moduleCode],
  );

  // MySQL returns array of rows
  if (!schemaDetail || schemaDetail?.length === 0) {
    throw new Error(`Schema not found for company ${companySlno}, module ${moduleCode}`);
  }
  const schemaName = schemaDetail[0].schema_name;
  // SECURITY CHECK REMOVE INVALID SECHEMA
  if (!/^[A-Z0-9_]+$/i.test(schemaName)) {
    throw new Error("Invalid schema name");
  }
  return schemaName;
};

/* --------------------------------------------------
   RETRY HELPER
-------------------------------------------------- */

const withRetry = async (fn, retries = 3, delayMs = 500) => {
  for (let i = 0; i < retries; i++) {
    try {
      return await fn();
    } catch (err) {
      if ((err.errno === 1213 || err.errno === 1205) && i < retries - 1) {
        console.warn(`⚠️ Deadlock detected. Retrying ${i + 1}/${retries}`);
        await new Promise((r) => setTimeout(r, delayMs));
      } else {
        throw err;
      }
    }
  }
};

module.exports = {
  startLog,
  endLogSuccess,
  endLogFailure,
  mysqlExecute,
  getCompanySlno,
  mysqlExecuteTransaction,
  getLastTriggerDate,
  getSchemaByCompanyAndModule,
  getLastProcessedAdmissionDate,
  getLastProcessedDischargeDate,
  captureDischargeHistory,
  withRetry,
};
